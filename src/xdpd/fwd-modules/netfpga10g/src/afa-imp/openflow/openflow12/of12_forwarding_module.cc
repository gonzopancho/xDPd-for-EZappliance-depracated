...
afa_result_t fwd_module_of12_set_pipeline_config(uint64_t dpid, unsigned int flags, uint16_t miss_send_len){
	...
	//Recover switch 
	lsw = physical_switch_get_logical_switch_by_dpid(dpid); // where this function is defined? how does it work?  ASK ABOUT IT!!!!
...
{																//WTF is going on in this function????
	of_switch_t* lsw;
	datapacket_t* pkt;
	ROFL_DEBUG("fwd_module_of12_process_packet_out \n");
	...
	if(!action_group_of12_packet_in_contains_output(action_group)){
		ROFL_DEBUG("PACKET OUT  ACTION GROUP \n");
	...
	if(buffer_id != OF12P_NO_BUFFER){
		ROFL_DEBUG("PACKET OUT  buffer_id  %d  \n",buffer_id,((struct logical_switch_internals*)lsw->platform_state)->storage);
		//Retrieve the packet
		datapacket_storage* storage;
		storage=((struct logical_switch_internals*)lsw->platform_state)->storage;
		pkt = storage->get_packet(buffer_id);
		

		ROFL_DEBUG("PACKET OUT  get_packet result  %d  \n",pkt);


		//Buffer has expired
		if(!pkt){
			ROFL_DEBUG("PACKET OUT FAILURE got empy buffer from buffer pool");
			return AFA_FAILURE; /* TODO: add specific error */
		}
	}else{
	...
	ROFL_DEBUG("Getting packet out [%p]\n",pkt);	
...
afa_result_t fwd_module_of12_process_flow_mod_add(uint64_t dpid, uint8_t table_id, of12_flow_entry_t* flow_entry, uint32_t buffer_id, bool check_overlap, bool reset_counts){
	...
	if(buffer_id != OF12P_NO_BUFFER){
		...
		of_process_packet_pipeline((of_switch_t*)lsw,pkt); // where this function is defined?
	}
...
afa_result_t fwd_module_of12_process_flow_mod_delete(uint64_t dpid, uint8_t table_id, of12_flow_entry_t* flow_entry, uint32_t buffer_id, uint32_t out_port, uint32_t out_group, of12_flow_removal_strictness_t strictness){
	...
	if(table_id == OF12_FLOW_TABLE_ALL){
		...
		//Single table
		if(of12_remove_flow_entry_table(lsw->pipeline, table_id, flow_entry, strictness, out_port, out_group) != ROFL_SUCCESS) //another function to define
			return AFA_FAILURE;
	}
	return AFA_SUCCESS;
} 
...
of12_stats_flow_msg_t* fwd_module_of12_get_flow_stats(uint64_t dpid, uint8_t table_id, uint32_t cookie, uint32_t cookie_mask, uint32_t out_port, uint32_t out_group, of12_match_t* matchs){
	...
	return of12_get_flow_stats(lsw->pipeline, table_id, cookie, cookie_mask, out_port, out_group, matchs); //another function to fill up
}
 ...
 * @brief   Instructs driver to add a new GROUP - what is a group?
 * @ingroup of12_fwd_module_async_event_processing
 *

...
static rofl_result_t netfpga_flow_entry_map_matches(netfpga_flow_entry_t* entry, of12_flow_entry_t* of12_entry){
	...
	ROFL_DEBUG("%s  %d num_of_matches: %x",__FILE__, __LINE__,of12_entry->num_of_matches);

	//Go through all the matches and set entry matches
	for(match = of12_entry->matchs; match;match = match->next){
		ROFL_DEBUG("%s  %d  of12_entry->type : %x, ",__FILE__, __LINE__, match->type);
		switch(match->type){
			...
			case OF12_MATCH_IN_PORT:
 			...
 			case OF12_MATCH_ETH_DST:
				...
				aux = (uint8_t*) &(((utern64_t*) match->value)->value);
				...
				*/		
				memset(&(masks->eth_dst),0x00,sizeof(masks->eth_src)); 
				...
				if(check_mac_mask(masks->eth_dst))
					//Is wildcarded
			...
			case OF12_MATCH_VLAN_VID:
 			...
 			case OF12_MATCH_IPV4_SRC:
				ROFL_DEBUG("of12_entry src_ip: %d ", ((utern32_t*)match->value)->value);
 			...
 			case OF12_MATCH_IPV4_DST:
				...
				ROFL_DEBUG("of12_entry dst_ip: %d ",((utern32_t*)match->value)->value);
